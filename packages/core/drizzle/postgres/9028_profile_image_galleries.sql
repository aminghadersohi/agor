...
CREATE UNIQUE INDEX "profile_images_one_primary_board_idx" ON "profile_images" USING btree ("tenant_id","board_id") WHERE "profile_images"."board_id" IS NOT NULL AND "profile_images"."is_primary" = true;--> statement-breakpoint
...
CREATE POLICY "tenant_isolation_profile_images" ON "profile_images"
  ...
  WITH CHECK ("tenant_id" = COALESCE(NULLIF(current_setting('agor.tenant_id', true), ''), 'default'));--> statement-breakpoint
