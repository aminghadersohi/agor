...
describe('MCP OAuth client-registration migrations', () => {
  ...
  it('follows current main and binds PostgreSQL authority to tenant/server UUID with forced RLS', async () => {
    ...
    expect(postgresJournal.entries.slice(-6)).toEqual([
      ...
      expect.objectContaining({ idx: 9018, tag: '9018_attention_watermark_collision_repair' }),
    ]);
    ...
    const attentionCollisionRepair = await readFile(
      new URL(
        '../../drizzle/postgres/9018_attention_watermark_collision_repair.sql',
        import.meta.url
      ),
      'utf8'
    );
    expect(attentionCollisionRepair).toContain('unrecognized partial session attention schema');
    expect(attentionCollisionRepair).toContain('session_attention_states');
    expect(attentionCollisionRepair).toContain('FORCE ROW LEVEL SECURITY');
  });
});
