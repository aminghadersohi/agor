    ...
    it('projects before decoding, retains hidden identity, and cannot read/count foreign boards', async () => {
      await runWithTenantDatabaseScope(db, tenantA, async (scoped) => {
        ...
        try {
          ...
          const { worktree_count, total_session_count, active_session_count, ...detail } = full[0];
          expect({ worktree_count, total_session_count, active_session_count }).toEqual({
            worktree_count: 0,
            total_session_count: 0,
            active_session_count: 0,
          });
          expect(await repo.findById(full[0].board_id)).toEqual({
            ...detail,
            worktree_count,
            total_session_count,
