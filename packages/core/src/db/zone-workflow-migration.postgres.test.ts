...
describe.skipIf(!enabled)('zone workflow PostgreSQL 9009 -> 9011 upgrade', () => {
  let db: Database;
  ...
  beforeAll(async () => {
    ...
    priorFolder = await mkdtemp(join(tmpdir(), 'agor-pg-migrations-through-9009-'));
    await cp(migrationsFolder, priorFolder, { recursive: true });
    await unlink(join(priorFolder, '9011_zone_workflow_transitions.sql'));
    const journalPath = join(priorFolder, 'meta', '_journal.json');
    const journal = JSON.parse(await readFile(journalPath, 'utf8')) as {
      entries: Array<{ idx: number }>;
    };
    journal.entries = journal.entries.filter((entry) => entry.idx <= 9009);
  ...
  it('preserves existing tenant data and installs usable forced-RLS workflow tables', async () => {
    // This isolated upgrade fixture has no daemon clients. Later authority and
    // collision-repair migrations require the same offline acknowledgement.
