...
test('drains a large single session in bounded transactions, preserves shared neighbors and keeps branch last', async ({
  ...
  const { branch: neighbor, user: neighborUser } = await seedEnvironmentCommandBranch(db);
  ...
  for (const [target, creator] of [
    ...
    if (target.session_id === session.session_id) {
      await executeRaw(
        db,
        sql`UPDATE session_reminders SET task_id = ${task.task_id} WHERE reminder_id = ${reminder.reminder_id}`
      );
    }
  }
  ...
  ).rejects.toThrow(/deletion|maintenance/);
  await expect(
    reminders.create({
      session_id: session.session_id,
      text: 'late reminder',
  ...
  expect(
    (await reminders.findPage({ session_id: foreign.session_id, limit: 10, skip: 0 })).total
  ).toBe(1);
  expect(
    (await memories.findPage({ session_id: session.session_id, limit: 10, skip: 0 })).total
  ).toBe(0);
  expect(
    (await reminders.findPage({ session_id: session.session_id, limit: 10, skip: 0 })).total
  ).toBe(0);
  expect(
