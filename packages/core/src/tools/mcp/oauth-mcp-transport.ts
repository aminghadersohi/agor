...
export async function resolveResourceMetadataUrl(
  ...
  options.assertCurrent?.();
  let advertisedMetadataMismatch = false;
    ...
    if (parsed) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (Array.isArray(data.authorization_servers)) advertisedMetadataMismatch = true;
        }
      } catch {
        // Keep the authority/deadline check outside the provider-error catch:
        // an expired daemon reservation is terminal, never another discovery
  ...
  if (advertisedMetadataMismatch) {
    throw new OAuthConfigurationError(
      'metadata_incompatible',
      'Protected resource metadata does not match the configured MCP endpoint'
    );
  }
      ...
      ? await options.resolveDynamicClientRegistration(
          {
            ...
            applicationType: oauthClientApplicationType(options.actualRedirectUri),
            scope: options.scope,
