    ...
    if (parsed) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          options.assertCurrent?.();
          mismatchedHint = !resourceMetadataMatches(
            parsed,
            data.resource,
            options.resourceUri,
            options.compatibilityMode
...
async function startMCPOAuthFlowWithAS(opts: {
  ...
  compatibilityMode: MCPOAuthRuntimeCompatibilityMode;
  dcrMode: MCPOAuthDCRMode;
  /** Loopback browser callbacks do not permit loopback provider egress. */
  ...
  // client metadata.
  assertSafeOAuthUrl(actualRedirectUri, {
    allowLocalhostHttp: opts.allowLoopbackRedirectUri,
  });
