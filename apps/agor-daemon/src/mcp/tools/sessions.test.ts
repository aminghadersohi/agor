...
describe('agor_sessions_list', () => {
  ...
  it('preserves auto-archive metadata on an authorized scoped page without exposing tokens', async () => {
    const app = makeFakeApp({
      branches: { get: async (id: string) => ({ branch_id: id }) },
      sessions: {
        find: async () => ({
    ...
    const parsed = JSON.parse((await agor_sessions_list({ branchId: 'wt-1' })).content[0].text);
