...
const ALLOWED_UNWRAPPED: Record<string, string> = {
  // processNextQueuedTask establishes queue tenant context and opens short
  // read/write units around admission. Its executor launch must stay outside
  // a call-site transaction; the MCP caller supplies trusted baseServiceParams.
  'sessionsService.triggerQueueProcessing':
    'processNextQueuedTask owns tenant-scoped queue admission',
        ...
        if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
          ...
          if (
            ...
            if (
              ...
              !(`${service.text}Service.${method}` in ALLOWED_UNWRAPPED) &&
