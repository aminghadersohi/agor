...
export function registerSessionTools(server: McpServer, ctx: McpContext): void {
  ...
  server.registerTool(
    ...
    {
      description:
        'Get a lean orientation snapshot for the current session in ONE call. Returns deduplicated context: session identity, user, latest task Git boundaries, branch (zone, issue/PR, notes, environment), board (with zones), repo (slug, default branch), genealogy, remote origins, and sibling sessions. Every field appears exactly once. Use get_current or entity-specific tools for full details. The returned session_id is what a remote orchestrator passes as callbackSessionId (agor_sessions_create) to self-target cross-branch completion callbacks. IMPORTANT: `genealogy`/`parent_session_id` describe BRANCH-LOCAL ancestry only. A session another session created cross-branch is legitimately `genealogy: "root"` with `parent_session_id: null` AND still has a durable origin — that origin is reported in `remote_origins`, so check it before concluding you have nobody to report to. Use agor_session_relationships_report to send it an explicit follow-up.',
      annotations: { readOnlyHint: true },
      inputSchema: z.object({
        includeSiblings: z
          .boolean()
  ...
  //
  // The explicit counterpart to the automatic completion callback. It exists
  // because the alternative — telling a remote-created session to reach its
  // origin with `agor_sessions_prompt(sessionId: <origin>)` — requires the
  // child to name an arbitrary session id, and is then authorized purely by the
  // caller's branch RBAC. This tool accepts NO target: it derives the
  // destination from the durable `remote_create` edge whose target is the
  // caller's own session, so "who may I reach" is answered by stored
  // provenance rather than by tool input. Branch RBAC still applies on top.
  server.registerTool(
    ...
    async (args) => {
      ...
      // Derive candidates from the caller's OWN session id. This — not a
      // filter applied to caller input — is what makes arbitrary targeting
      // impossible: an unrelated relationship simply never enters this list.
      ...
      if (args.relationshipId) {
        ...
        if (!requested) {
          // Deliberately does not distinguish "no such relationship" from
          // "exists but is not yours": either way the caller learns only about
          // its own origins.
          return {
            ...textResult({
      ...
      // Read the origin through the caller's own RBAC/tenant scope first. A
      // relationship row can outlive what it points at (or point somewhere the
      // caller cannot see), and a report is worth refusing with a reason rather
      // than delivering into the void.
      ...
      // Refuse rather than deliver. The prompt route auto-unarchives its
      // target, so proceeding here would silently resurrect a session someone
      // deliberately archived on the strength of a background agent's report.
      ...
      // Same authorization the callback-registration paths use
      // (agor_sessions_create / agor_sessions_prompt callback:true), so the
      // explicit route cannot become a way around branch RBAC, tenancy, or the
      // personal-session-sharing grant. The prompt route below re-checks this
      // through the normal hook chain as well.
      await runWithMcpTenantDatabaseScope(ctx, (db) =>
        ensureCanPromptTargetSession(
      ...
      return textResult({
        ...
        // Reported, never changed: an explicit report is not a subscription.
        automatic_completion_callback_enabled: relationship.callback_enabled,
        note:
          task.status === 'queued'
