...
function makeStartupContextWithGuardedDb(fixtures: StartupFixtures = {}) {
  ...
  const ctx = {
    ...
    // This fork owns UPS power policy for the whole startup/shutdown span, so
    // every full `startup(ctx)` needs the controller the daemon always sets.
    powerPolicyController: {
      start: vi.fn(),
      stop: vi.fn(),
      status: () => ({ ownership: 'owned', held: false }),
      subscribe: vi.fn(() => vi.fn()),
      markRecoveryQueueDrained: vi.fn(),
      assertOwnershipInTransaction: vi.fn(async () => {}),
      closeOwnership: vi.fn(async () => {}),
    },
