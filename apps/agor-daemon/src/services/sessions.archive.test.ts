...
describe('SessionsService archive routes', () => {
  ...
  dbTest(
    ...
    async ({ db }) => {
      ...
      // Prompt admission owns this narrow deadline cancellation. Archive state
      // itself is intentionally reserved for the dedicated lifecycle methods.
      await service.patch(child.session_id, { auto_archive_at: undefined });
