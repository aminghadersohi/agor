...
describe('configured executor spawning', () => {
  ...
  it.runIf(process.platform === 'linux')(
    'carries the per-user home store through local request handoff into bubblewrap',
    async () => {
      const root = realpathSync(mkdtempSync(path.join(tmpdir(), 'agor-owner-home-handoff-')));
