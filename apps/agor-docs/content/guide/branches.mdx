...
### Browse and edit worktree files

Open a branch's **Files** tab to browse or download its files. **Open workspace editor** expands
that browser into a resizable, IDE-style view with the file tree on the left and an editor on the
right. Save with the button or `Cmd/Ctrl+S`.

The editor updates the worktree directly. It supports existing text files up to 1 MB and refuses a
save if the file changed after you opened it, so an agent or external editor is not silently
overwritten. Binary files remain preview/download-only. In RBAC workspaces, editing requires
filesystem `write` access to the branch.
...
If two branch records share or overlap a workspace path, choose **Archive → Leave
untouched** to hide the broken record without touching files. Cleanup, workspace
removal, and permanent deletion remain blocked until ownership is reconciled,
even if the other record is archived, failed, cleaned, or marked deleted. These
states do not prove that files are absent or that provisioning cannot retry.
...
One durable Branch-row claim excludes competing work; duplicate active requests
are rejected, not dispatched again. Built-in cleanup is bounded to five minutes;
...
V1 checks known task, upload and environment activity. Terminal closure is best-effort;
detached/unmanaged processes are **not proven stopped**. Delegated/external execution
is rejected until its containment is supported. Branch `updated_at` is metadata
activity, not evidence of filesystem inactivity. There is no scheduler or age cutoff.
...
Deletion currently requires a local managed executor and available authoritative
storage roots; delegated or external launchers are rejected. Historical SDK sessions
using a shared execution home also block deletion until their owned storage is
reconciled; the shared user home is never recursively removed. Remote Git refs and
