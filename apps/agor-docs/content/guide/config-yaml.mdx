...
The Essential Session selector lists authorized Sessions, including archived Sessions, and
identifies an existing Essential Session only if it is visible to the caller. The selector keeps
server-side pages bounded at 50 records and shows its compact pager only when more than one page
exists. It uses
