...
Agor reads `~/.agor/config.yaml` as **deployment-owned, immutable runtime input**. Set
[`AGOR_HOME`](#the-agor-home-agor_home) to relocate that directory. `agor init`
...
Daemon-driven **Sign in with Claude** is available by default only in supported,
operational execution modes. Explicit opt-out wins:

```yaml
agentic_tools:
  claude_subscription_oauth: false
```

Opt-out stops new managed sign-ins and backend token resolution/refresh; it does
not disable ordinary Claude tasks using API keys or pasted subscription tokens.
Disconnect remains available for saved backend logins even after opt-out or a
capability downgrade.

Two storage modes are supported:

- **Local file:** the existing contained sandbox with persistent per-user homes,
  `sandbox.enabled: true`, `sandbox.home_mode: per_user`, no command template and
  no `extra_allow_write`. The actual isolation probe must pass. HA additionally
  requires shared-local topology and cross-replica flock. The daemon alone reads
  and refreshes the canonical `.claude/.credentials.json`; immutable parent
  mounts and authority-leaf masks keep task runtimes away from it. Local SQLite
  behavior is unchanged.
- **Backend:** PostgreSQL, the existing daemon master-secret bootstrap, delegated
  execution with a reviewed command template, persistent-per-user identity
  routing, and a validated `executor-response-v1` channel with replica-exact
  origin. HA additionally requires the validated external topology and durable
  task-token authority. The daemon encrypts personal tenant/user grants in the
  runtime DB. Refresh tokens never enter executor/helper pods, homes, launch
  payloads, browser responses or realtime events. Only the access token for the
  live task actor reaches that task through the existing authenticated credential
  RPC. No local file, flock or fake sandbox setting is required for this mode.

Simple execution, SQLite backend storage, missing durable authority and unknown
external protocols fail closed. `features.claudeOAuthCapability` reports
`available`, `storage` (`backend`, `local_file`, or `null`) and a safe reason;
`features.claudeSubscriptionOAuth` remains the compatibility boolean. Technical
readiness does **not** establish provider approval. The release owner must obtain
approval for offering third-party Claude login before enabling a real service.

**Upgrade sequencing:** on an eligible deployment, upgrading the image with this
setting omitted enables sign-in. Technical compatibility with Cloud is not
release clearance. Before installing this image without provider approval,
operators must explicitly set `claude_subscription_oauth: false` in their
version-controlled deployment configuration. Enable it only in a separately
authorized rollout; do not rely on an older image's default-off behavior.

Refresh claims are database-fenced across replicas. An uncertain exchange is not
retried with the possibly consumed refresh token; reconnect in Settings. The
backend status poll reports saved/usable state without refreshing or spawning an
auth helper, and does not claim successful provider validation. A tenant write
freeze refuses new refresh claims and dispatch. If a freeze begins after
dispatch, the claim stays unresolved rather than writing through the freeze;
after thaw it is conservatively settled as ambiguous, never replayed.

Access is refreshed before SDK launch, not inside an already-running SDK. A long
task or permission wait can outlive its token. After authentication expiry, send a
follow-up prompt in the same session to create a new task with a fresh token and
existing SDK resume state.
Agor does not replay prompts automatically or switch billing sources. Disconnect
cannot recall an access token already delivered; authorized work may continue
until provider expiry or normal Stop. Cloud's existing internal HTTP channel
relies on the trusted cluster network, not mTLS; verify network policy before
rollout.

Migration `0110_user_provider_oauth_grants` ships for both database dialects and
requires an **offline cohort cutover**: drain tasks/attempts, stop all old writers,
snapshot the DB, run `agor db migrate --offline-cutover --yes`, then start only
compatible replicas sharing the existing master secret. Do not import native
credential files into backend grants; users explicitly reconnect. Grants and
attempts are non-portable: tenant re-home requires reauthorization.

The cutover is a credential-protocol fence, not just a table-DDL requirement.
Older replicas do not understand the durable `managed_oauth` selection or the
version-2 backend attempt material. In particular, an older resolver can treat
that selection as missing personal credentials and fall through to a workspace
credential under a preferred policy, rather than preserving the chosen billing
source. Keep old writers/readers out of the cohort; additive DDL alone does not
make mixed versions safe.

For rollback, opt out first and keep compatible binaries/schema. A binary older
than this migration requires an offline drain and a tested backup or coordinated
schema/ledger rollback; never bypass startup watermark guards. A restored backup
can hold provider-invalidated rotating tokens, so require reconnect rather than
replaying historical grant state. Never export backend grants into SDK files.
...
## UPS-aware dispatch admission (macOS V1)

Power conservation is off by default. On a supported standalone macOS host, the privacy-filtered,
read-only provider continues to report whether a UPS is detected and whether the host is on utility
or battery power; **Off** means only that Agor does not gate Sessions or schedules. Start with
`observe`, verify the admin power-source indicator and monitoring workspace through several normal
samples, and only then choose `enforce`:
...
execution:
  ...
  power_management:
    ...
    on_battery_debounce_ms: 10000
    online_stable_ms: 60000
    recovery_dispatch_interval_ms: 2000
...
The provider invokes Apple's fixed, read-only `/usr/bin/pmset -g ps` command without a shell. It
retains only normalized power condition, optional charge/runtime, freshness, and a bounded reason
category. Device names and raw output are not stored, logged, or returned by the API.

`observe` derives the complete policy but never gates. `enforce` holds ordinary Task dispatch while
on UPS battery, while status is unknown, and during stable-online recovery. Existing running Tasks
are never stopped. One Branch Manager-selected Essential Session may continue to dispatch while the
policy is Conserving. Critical and stable recovery hold every new turn. Queued prompts stay durable;
after 60 seconds of stable utility power they resume at a paced rate. Schedules do not materialize
occurrences while held and materialize only the latest missed occurrence once after recovery.

This first version rejects active configuration unless the deployment is macOS, standalone,
static-tenant, local-executor, and `unix_user_mode: simple`, with either SQLite or
explicitly owned standalone PostgreSQL (see below). It does not control host
shutdown, suspend processes, use `SIGSTOP`/`SIGCONT`, stop running Tasks, or provide NUT/network UPS
polling. Provider identity and deployment topology remain operator-managed in `config.yaml`.
Mutable policy values—mode, thresholds, timing, and recovery pacing—can be applied live from the
admin workspace. Queued Tasks and Session priority metadata remain intact through every policy
change. A provider/topology change still requires an operator edit and daemon restart because those
values determine whether host observation is safe to construct at startup.

### Standalone PostgreSQL: exclusive host ownership

This is **not HA**. It supports one static tenant's local execution on one Mac.
It does not support dynamic tenants, multiple execution hosts, sandbox/delegated
execution, executor command templates, nonmacOS providers, or transaction-pooling
proxies. Use a direct PostgreSQL connection (including the extra dedicated ownership
connection) and an application role with `NOSUPERUSER NOBYPASSRLS`.

The operator must provision a new UUID, for example with `uuidgen`, and retain it
in the host's operator-owned configuration:

```yaml
deployment:
  mode: standalone
  standalone_power_host_id: "00000000-0000-4000-8000-000000000001" # fictional; generate your own
database:
  dialect: postgresql
multi_tenancy:
  mode: static
...
The UUID is an **operator assertion of host identity**, not hardware attestation or
a hostname. Never copy it to a different machine or provision different identities
for competing daemons on the same deployment. The database durably binds the tenant
to its hash. Losing that configuration does not authorize an automatic replacement;
retain it in the operator's recovery inventory. Cross-host migration/rebinding is
not supplied by this feature, including after a crash or database restore. Importing
or copying the tenant archive also copies this binding; it is not a host-transfer
procedure. Only normalized `owned`/`lost` status is exposed to the UI—never the UUID,
hash, boot identifier, or backend identifier.

Ownership is established before initial data setup or runtime startup. An exclusive
acquisition lock rejects competitors; the owning dedicated PostgreSQL session then
holds a shared lock. Each durable Task dispatch, schedule occurrence admission, and
policy mutation holds its own shared **transaction** lock and checks both the durable
boot binding and the owning backend's live lock. A replacement cannot overtake an
in-flight admission, even if the ownership connection disconnects. The mutation uses
the actual tenant transaction, not an unrelated pool checkout. Existing unbound
PostgreSQL deployments are not silently opted in, but upgraded claimers without the
matching owner cannot admit against an already-bound tenant.

**Ownership is independent of conservation.** Loss blocks new admissions and policy
Apply even in Off or Observe. Ordinary reads and running Tasks are not stopped or
suspended by this mechanism. The failed controller never reacquires automatically.
The UI refreshes ownership separately from the UPS source and policy mode; an
`owned` display is only a recent observation, never the authority used by a claim.

For an intended same-host restart, stop the old daemon first and wait for its ownership
connection and in-flight admission transactions to close, then start the new daemon
with the **same identity**. A new boot is accepted only when the old lock is gone.
There is no lease expiry or automatic cross-host takeover. If a severed connection
remains alive at PostgreSQL, restart refuses rather than assuming the old owner died.
The owned topology uses the existing non-destructive PostgreSQL Task startup/shutdown
policy: it does not rewrite running Tasks as restart orphans, intentionally terminate
executors at daemon shutdown, retry prompts, or replay a committed dispatch. Executor
survival still depends on the operator's process supervisor; ownership does not prove
external work stopped. Ordinary Task runtime supervision remains separate.

#### Offline upgrade, verification, and rollback

PostgreSQL migration `9020_standalone_power_ownership` is a protocol-only cutover.
It reuses forced-RLS `app_variables` and the existing tenant-scoped unique Essential
index; there are **no new schema columns or tables**, and no SQLite migration.
Both migration journals retain their independent ordering. Old binaries do not
participate in this protocol, so mixed-version operation is unsupported.

1. Back up the database and operator configuration. Stop **every** daemon/claimer
   using that database; do not rely on the power mode to drain or stop running work.
2. Using the target release's CLI and its normal operator database configuration,
   inspect `agor db status --json`, then run `agor db migrate --offline-cutover`.
   From a source checkout these commands are `pnpm agor db status --json` and
   `pnpm agor db migrate --offline-cutover`. The command implementation lives in
   `apps/agor-cli/src/commands/db/`; cutover policy is owned by
   `packages/core/src/db/migrate.ts`. No credentials belong in command history.
3. Provision the immutable identity only on the intended Mac and start **one**
   upgraded daemon. Verify exclusive ownership and the actual effective policy:
   a pre-existing durable override takes precedence over YAML, including `mode`.
   Do not assume a YAML `off` value reset the runtime policy.
4. In the admin workspace, inspect the revision/source, then Apply **Observe**.
   Verify normal, read-only UPS samples, freshness, and ownership without inducing
   an outage. Confirm ordinary scheduling/admission remains available. Enforce is
   a separate, deliberate operator action after Observe verification.
5. For policy rollback, Apply Off or reset the override to known-safe operator
   defaults while ownership is healthy. This does not release host ownership.
   After ownership loss, stop and restart on the bound host before policy changes.
   Removing the identity or choosing Off does not unbind the database.

Binary rollback to a pre-ownership release is incompatible with the new protocol
marker: do **not** edit migration history or delete ownership rows to make an old
binary start. Restore the pre-cutover database/configuration backup offline only
with all daemons stopped and previously running external work accounted for; a
restore is not authorization to replay prompts. Otherwise stay on the upgraded
binary with conservation Off. This feature provides no live rebind, takeover,
shutdown, or rollout automation.

### Admin configuration and monitoring UI

Admins can open **Settings → Admin → UPS Power**, or select the persistent power indicator in the
app header. Provider facts and policy mode are always separate: the workspace reports **UPS
detected**, **Utility power**, **Battery power**, **Provider unavailable**, **Provider stale**, or
**Unsupported topology**, while Power conservation independently reports **Off**, **Observe**, or
**Enforce**. Off never means that a UPS is disconnected. Members and viewers cannot read host
telemetry or open this pane.

The workspace shows the active mode and supported provider, normalized source, provider health,
optional charge/runtime, freshness, reason, UTC timestamps, ordinary and Essential admission, and
stable-online recovery and queue pacing. Reads refresh every five seconds and on policy transitions.
A failed refresh clears the displayed readings rather than presenting old telemetry as current.
Missing telemetry is shown as **Not reported**, never zero. Held denotes an admission decision;
there is no held-task count or inventory API.

The configuration editor is initialized from the active policy and applies valid mutable values
without a daemon restart. A policy change starts fresh policy evaluation rather than treating an
old provider sample as newly observed; Enforce remains conservative until a new sample and any
required stable-online recovery interval complete. An admin update uses an exact revision compare-and-swap: if another tab
or administrator has already changed the policy, the stale update is rejected and realtime status
converges both tabs on the newer revision. The durable, tenant-owned runtime override is loaded on
every daemon start and takes explicit precedence over mutable operator defaults. Agor never rewrites
`config.yaml`. **Reset to operator defaults** removes the override as a new audited revision;
**Roll back last change** restores the immediately preceding policy as another audited revision.
Rollback stores an exact policy snapshot, so a later operator-default edit cannot change its
meaning. Unsupported active topology is rejected before persistence. No provider addresses,
commands, device names, raw output, or credentials are exposed.

The Essential Session browser has no pager and does not fetch an unbounded inventory. It shows the
current selection separately, then offers up to 30 recent eligible Sessions and a debounced,
server-side title or Session-ID search for older Sessions. Archived Sessions and Sessions on
archived Branches are excluded. Results contain only the minimal Session projection after tenant and
Branch Manager authorization; an inaccessible existing selection is reported only as an occupied
slot, without revealing its title or status. The browser does not poll, count, traverse hidden
pages, or issue per-result lookups. It uses the existing atomic priority API for changes. Admin role
alone does not grant foreign-branch access. Clear an existing Essential designation before choosing
a replacement; the UI never silently demotes an inaccessible Session.
...
## The Agor home (`AGOR_HOME`)

Agor keeps its state in one directory: `config.yaml`, the standalone SQLite database,
`admin-credentials`, logs, the managed agentic-tool tree, and runtime files. That directory
defaults to `~/.agor` and is relocated wholesale by the `AGOR_HOME` environment variable:

```bash
AGOR_HOME=/srv/agor-staging agor daemon start
```

Resolution is `AGOR_HOME`, else `<home directory>/.agor`. A value may be absolute or start with
`~`, and is resolved to an absolute path; a relative value resolves against the process working
directory. A value that is empty or only whitespace is treated as **unset** rather than as `/`, so
an `AGOR_HOME=` line in an env file cannot redirect Agor's state to the filesystem root.

Every Agor-owned path derives from this root, so they always move together — there is no
configuration in which `config.yaml` is read from one home and the database from another. Set
`AGOR_HOME` to run a second, throwaway daemon without it reading or writing the state of the
deployment already installed under `~/.agor`.

`AGOR_DATA_HOME` (or `paths.data_home`) still separates the *git data* — `repos/` and `worktrees/` —
from the daemon's operating files, and defaults to `AGOR_HOME`. `AGOR_DB_PATH`, `DATABASE_URL`, and
`database.sqlite.path` still override the database location specifically; a `~` in any of those is
the operating-system home, not the Agor home.

## Environment precedence

Supported environment overrides still win over YAML. Important examples are `AGOR_HOME`,
`AGOR_DATA_HOME`, `PORT`,
...
<a id="choosing-a-mode" />
<a id="supported-connections-and-limits" />
<a id="diagnostics" />
<a id="enabling-mediation-and-rolling-back" />

## MCP connection security

MCP security modes are **tenant-owned API settings**, not `config.yaml` keys.
See [MCP Administration](/guide/mcp-administration#mcp-connection-security) for
mode selection, diagnostics, supported connections, and safe rollout or rollback.
...
- Validate destination delivery and schema after rollout. Agor does not migrate
  warehouse data, backfill historical events, configure live destinations, or update
  dashboards for you.
