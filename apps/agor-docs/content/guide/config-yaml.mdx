...
MCP OAuth uses a native-tool loopback callback by default, including when a standalone daemon uses
PostgreSQL. This needs no public DNS and expects the sign-in browser to run on the daemon host. A
single daemon used through a browser on another machine can opt into its configured public HTTPS
origin:

```yaml
daemon:
  base_url: https://agor.example.com
  mcp_oauth_callback_mode: public
```

Register `https://agor.example.com/mcp-servers/oauth-callback` with the provider and route that path
through the reverse proxy to the daemon. `AGOR_BASE_URL` can supply the base URL instead; callback
mode remains an explicit YAML choice.
...
## Optional restart continuation

Standalone installations can opt into slowly resuming work that a daemon
restart interrupted:
...
execution:
  restart_recovery:
    enabled: true
    delay_ms: 2000
    max_tasks_per_start: 50
    resume_after_crash: false
```

Agor never replays an interrupted prompt. It terminalizes the old Task, records
a durable pending marker, and creates a new system-authored continuation that
instructs the agent to inspect prior state before proceeding. Admissions are
sequential and deterministic, so retries do not create duplicate turns. A user
prompt or archive supersedes pending recovery; Tasks stopped intentionally or
waiting for permission/input are excluded.

`resume_after_crash` is a separate, higher-risk opt-in because an abrupt local
daemon loss cannot prove the old executor is contained. Leave it disabled
unless the execution substrate guarantees process loss with the daemon. This
feature is disabled by default and is not used in shared-replica mode. Rollback
is configuration-only: disable `restart_recovery`; existing terminal history
and continuation Tasks remain ordinary auditable Tasks.
