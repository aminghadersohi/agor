...
- **Strict MCP OAuth by default**: Agor binds protected-resource and authorization-server metadata, sends the protected `resource`, requires PKCE S256, and validates `iss` whenever the provider returns it. RFC 9207's support flag is optional; when a provider advertises it, Agor requires `iss` in the callback. A standalone daemon uses an exact loopback callback without public DNS regardless of database dialect; a remote-browser deployment can explicitly select a configured public HTTPS callback. A narrow **Legacy compatibility** setting enables non-standard discovery only for a server that needs it; Agor never silently downgrades.
