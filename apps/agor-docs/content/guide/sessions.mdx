...
Forks and spawned children copy selection names for continuity, never stored values. Each child resolves the copied names against the child session's own attributed owner. With `dangerously_allow_session_sharing` off (the default), a cross-user child is attributed to the caller, so a copied name cannot borrow the parent's secret. Callbacks carry message content, not an environment grant.
