...
- `execution.managed_envs_execution_mode: webhook-only`, or the explicitly configured [bounded external command profile](/guide/environment-configuration#bounded-external-commands-in-ha); and
...
origin on every replica. The checked-in `.agor.yml` `ha` variant honors an inherited value (needed
when a public HTTPS proxy fronts the branch ingress), or otherwise renders a development HTTP value
from Agor's host address and branch port. OAuth on PostgreSQL/HA requires the former to be a safe
browser-reachable HTTPS origin; the development HTTP default cannot start a provider flow.
Without that public origin, `mcpOAuth` reports false and OAuth start refuses before provider
discovery. `daemon.public_url` is reserved for internal daemon/executor callbacks and is never an
OAuth browser redirect origin.
...
The development issuer also signs `public_base_url` from trusted fixture configuration,
snapshotted together with `iat` when the launch code is created. Optional
`AGOR_HA_ACME_ORIGIN` and `AGOR_HA_GLOBEX_ORIGIN` select each tenant's origin;
both default to `AGOR_HA_PUBLIC_ORIGIN` in plain Compose. Redirects use the selected
tenant origin, and both replicas allow exactly these configured origins for CORS.
No request Host header or submitted URL chooses the signed origin.

The managed `ha` template uses the main ingress port for Acme and a second
`HA_TENANT_PORT` (branch ID offset 15000) for Globex. Both ports reach the same
nginx/replica pool. This permits separate browser storage and catches origin
mixups without requiring DNS. To test different **hostnames**, explicitly configure
two names that resolve to that ingress (and arrange browser DNS/TLS as needed).
The deployment/OAuth origin remains `AGOR_HA_PUBLIC_ORIGIN`.

After starting the disposable environment, run the focused, model-free routing smoke:

```bash
AGOR_HA_INTEGRATION=1 HA_URL=http://127.0.0.1:3030 \
  HA_DAEMON_A_PORT=13031 HA_DAEMON_B_PORT=13032 \
  AGOR_HA_ACME_ORIGIN=http://localhost:3030 \
  AGOR_HA_GLOBEX_ORIGIN=http://localhost:3031 \
  node scripts/test-ha-tenant-links.mjs
```

For that example, start Compose with `HA_TENANT_PORT=3031` and those two tenant
origins; otherwise its second published port is dynamically allocated. Supply
the same origins used when starting Compose. The test logs in both fixture
tenants and checks interleaved board URLs on both replicas, tenant isolation, exact
CORS, and deep-link HTML delivery. It does not run models, restart containers, or
prove browser rendering or Cloud workspace resolution. Existing environments need
one new picker login per tenant to initialize routing metadata. Agor stores repo
environment templates separately: editing `.agor.yml` alone does not refresh a
previously imported template. For branch-only testing with an older template, the
non-secret tenant origins and `HA_TENANT_PORT` can be supplied in a gitignored
Compose `.env` file without changing the shared repo registration.
...
| Managed environments                               | Webhook-only lifecycle behavior remains supported. External delegated hybrid additionally supports bounded attempt-scoped Start/Stop/Nuke with executor-initiated reporting; see the [configuration and deadline contract](/guide/environment-configuration#bounded-external-commands-in-ha). Restart is unavailable for that profile. Health observation runs on every replica with bounded discovery and one PostgreSQL lease/token per branch. The result write rechecks DB-time lease expiry, opaque token, active status, archive state, tenant, and lifecycle generation. Missing health URLs are recorded as `unknown`; Agor does not infer remote health from another replica's process map. Authenticated health reports `environmentHealthMonitor: true`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
> daemon-issued short-lived-token containment. The Codex runtime remains a
> credential-file writer; generation fencing does not provide canonical-file
> containment.
...
The opt-in harness exercises the checked-in HA Compose project: replica readiness,
REST distribution, Socket.IO affinity and authorized delivery, tenant and private-board
access denial, credential-attempt coordination, and daemon/Redis loss and recovery.
It uses simulated credentials, not real provider sign-in. Run it only against the
disposable development stack, not a production deployment.
...
Passing this harness is a transport, control-plane, and tenant-boundary smoke
result, not full product HA certification. It does not exercise real provider
authentication, prove that an in-flight executor survives daemon loss, validate
Cloud’s external execution substrate, cover every gateway provider or task
permission mode, or make blocked process-affine features HA-safe.
