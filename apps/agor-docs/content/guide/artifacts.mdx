...
- `agor_runtime`: Enables agent DOM introspection via `agor_artifacts_query_dom` by default. Opt out per artifact with `{ enabled: false }`, especially when rendering sensitive output. It also carries the artifact's declared [interaction bindings](#interaction-bindings) under `agor_runtime.interactions`.
...
- **Controls**: When the artifact declares [interaction bindings](#interaction-bindings) and you can view its branch, a toolbar under the preview shows each data binding's status and a button per action and chat.
...
An artifact can be a small control panel for its branch: show whether a schedule is armed, run it once, arm or disarm it, or open a conversation. It does this through **bindings** the author declares when publishing. Artifact code never gets a token and can't call Agor's API directly; it can only name a binding by id.

There are three kinds:

| Kind      | Declares                                                           | Artifact code calls         |
| --------- | ------------------------------------------------------------------ | --------------------------- |
| `data`    | A read of a schedule's or session's status (a fixed set of fields) | `window.agor.fetchData(id)` |
| `actions` | Run a schedule once, or arm / disarm it                            | `window.agor.runAction(id)` |
| `chats`   | A session to open in Agor                                          | `window.agor.openChat(id)`  |

Declare them with `interactionConfig` on `agor_artifacts_publish` or `agor_artifacts_update`:

```ts
agor_artifacts_update({
  artifactId,
  interactionConfig: {
    data: [{ id: 'nightly', label: 'Nightly', kind: 'schedule_status', scheduleId }],
    actions: [
      { id: 'run-now', label: 'Run once', scheduleId, confirm: true },
      { id: 'disarm', label: 'Disarm', scheduleId, effect: 'disable' },
    ],
    chats: [{ id: 'triage', label: 'Triage', sessionId }],
  },
});
```

Over REST, the same bindings are the `agor_runtime.interactions` field of `PATCH /artifacts/:id`, in the stored shape (`effect: { kind, schedule_id }`, `source: { kind, schedule_id | session_id }`, `session_id`).

**Rules Agor enforces when you save:**

- Every schedule and session a binding names must be on the artifact's own branch.
- Ids are 1–64 letters, digits, `_` or `-`, unique within their kind; each binding needs a `label`.
- Bindings take no arguments. Everything, including whether an action arms or disarms, is fixed when you declare it.
- Invalid bindings are rejected with the reason and nothing is saved.

**Who can use them.** Bindings run as the person viewing the artifact, with their own permissions. They're a shortcut, not a grant: a viewer who couldn't run the schedule by hand gets the same refusal from the button. Viewers who can't view the artifact's branch see the artifact without controls. Actions declared with `confirm: true` ask for confirmation in an Agor dialog the artifact can't draw or dismiss.

Agor renders the bindings as a toolbar on the artifact card and the fullscreen page, so they work even if the artifact's own code never calls `window.agor`. `window.agor` needs the runtime (`agor_runtime.enabled`, on by default).

To repoint a chat or action later, call `agor_artifacts_update` with new `interactionConfig`; the artifact's code doesn't change. `interactionConfig: null` removes all bindings.
...
Only the `react` / `react-ts` environment mapping has been verified with a live artifact. The Vue, Svelte, Solid, and Angular mappings are best-effort: validate them with your template before relying on environment injection. Static and vanilla templates do not support it.
