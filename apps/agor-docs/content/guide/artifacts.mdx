...
Under the hood, artifacts are powered by [Sandpack](https://sandpack.codesandbox.io/), the open-source in-browser bundler by [CodeSandbox](https://codesandbox.io/). A directory of source files inside the branch gets compiled and rendered live in the browser, with no server-side build step needed. The agent writes code, the board shows the result.

---

## How it works

1. **Agent writes a folder** of UTF-8 text source files (a regular `package.json`, `App.tsx`, etc.), typically inside the branch so the code is version-controlled. Sandpack's file map is text-only: embed local images/fonts as data URLs in source or use controlled external URLs rather than placing binary files in the folder.
2. **Agent calls `agor_artifacts_publish`** with the folder path, the board, the Sandpack template, and (optionally) declarative metadata: `requiredEnvVars`, `agorGrants`, `sandpackConfig`, and `interactionConfig`. The daemon serializes the file map into the database.
...
- `agor_grants`: Declarative daemon capabilities (`agor_api_url`, viewer email, and metadata grants).
- `agor_runtime`: Controls injection of the daemon-side `agor-runtime.js` (which powers agent DOM introspection and declared actions). Enabled by default; opt out per-artifact with `{ enabled: false }`. The runtime ships as an iframe-level `<script>` tag via Sandpack's `externalResources`, so it works regardless of which user files exist (no entry-file detection, no file mutation, no template-specific gotchas).
- `agor_runtime.interactions`: Server-validated data, action, and chat bindings. Data bindings are read-only projections; action bindings reference existing schedules, which remain the source of truth for prompts, models, tools, and execution policy; chat bindings reference canonical session IDs rather than mutable titles. Every binding target must live on the artifact's source branch.
...
| Tool                                | Purpose                                                                                                                                                                                                               |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `agor_artifacts_publish`            | Publish a folder as a new artifact, OR re-publish an existing one with `artifactId`. Can declare schedule-backed actions and a chat binding through `interactionConfig`.                                             |
| `agor_artifacts_create_chat`        | Create a private chat artifact bound to one canonical session UUID on that session's board                                                                                                                           |
...
| `agor_artifacts_update`             | Patch metadata only (board move, name, `requiredEnvVars`, `agorGrants`, `sandpackConfig`, `interactionConfig`)                                                                                                        |
...
| `agor_artifacts_export_codesandbox` | Eject the artifact to CodeSandbox (returns a sandbox URL). Daemon-supplied capabilities won't work there, so set required env vars via CodeSandbox Secret Keys                                                        |
...
### Data, action, and chat bindings

An artifact can read Agor state and drive schedules without ever receiving an API token, naming a route, or choosing a prompt at runtime. Declare each binding with an artifact-local ID:
...
agor_artifacts_publish({
  ...
  interactionConfig: {
    // Reads. window.agor.fetchData('nightly')
    data: [{ id: 'nightly', label: 'Nightly build', kind: 'schedule_status', scheduleId }],
    // Writes. window.agor.runAction('review')
    actions: [
      { id: 'review', label: 'Run release review', scheduleId, confirm: true },
      { id: 'arm', label: 'Arm nightly', scheduleId, effect: 'enable' },
      { id: 'disarm', label: 'Disarm nightly', scheduleId, effect: 'disable' },
    ],
    // Chats. window.agor.openChat('triage')
    chats: [{ id: 'triage', label: 'Release triage', sessionId }],
  },
});
```

Artifact code calls `await window.agor.fetchData('nightly')`, `await window.agor.runAction('review')`, or `await window.agor.openChat('triage')`.

**Each call names a binding `id` and nothing else** — no tool name, no route, no arguments, no credentials. The daemon re-reads the persisted binding, re-checks that its target is still on the artifact's source branch, and then dispatches through the ordinary authenticated service path **as the viewer**. An artifact button is a shortcut, not a capability: if you could not run that schedule by hand, the button fails the same way.

Consequences worth knowing when you author one:

- **Arguments are pinned at publish time.** A toggle is two declared actions (`effect: 'enable'` and `effect: 'disable'`), not one action the artifact parameterizes. `effect` defaults to `'run'`, a one-off run.
- **Reads cannot write.** Reads and writes are separate collections on separate transports, so a read id does not exist in the actions list at all — and a read is a `GET` with no body.
- **Reads are projected.** `schedule_status` returns name, enabled, cron, and run timestamps; `session_status` returns title, status, and agent. The underlying `prompt` and agent configuration never reach the artifact.
- **Everything stays on one branch.** Schedules and sessions referenced by any binding must belong to the artifact's source branch; cross-branch references are rejected at publish.
- **Repoint a chat with metadata, not code.** `agor_artifacts_update` can swap a `chats` entry's `sessionId` to retire a conversation and replace it with another, without republishing the artifact.
- **`openChat` opens the parent surface.** The real conversation renders in Agor's own session panel with its full controls; it is never embedded in the iframe, which would require giving the iframe a token.

MCP servers are deliberately not bindable. An artifact that needs MCP data should point an action binding at a schedule whose agent has that server attached, and read the result the schedule writes back. The reasoning is in [`context/explorations/artifact-interaction-bindings.md`](https://github.com/preset-io/agor/blob/main/context/explorations/artifact-interaction-bindings.md).

Use the pin control in an artifact header to add it to **Home**. Pinned action buttons run directly from Home, while chat pins open the normal live session surface with its full controls. Use the move control to place the artifact on another board; an artifact has one canonical board placement rather than duplicated mutable copies.
...
- `agorGrants`: capabilities the daemon will inject (daemon URL, viewer email, and artifact metadata).
...
| `static`                  | Static browser server                | n/a           | not supported             | verified                             |
| `vanilla`, `vanilla-ts`   | Static / Parcel                      | n/a           | not supported             | verified                             |

For new HTML/CSS-first artifacts, use the `static` template with `/index.html` as the entry and keep browser JavaScript in linked or inline scripts. Existing `vanilla` artifacts with a present, empty (or comment-only) conventional JavaScript entry are repaired to Sandpack's `static` template automatically. This keeps the original `index.html` markup, inline styles, linked CSS, and scripts intact while preserving the normal vanilla path for artifacts with executable JavaScript. Missing or alternate entry paths are not guessed at.
...
const agorApiUrl = process.env.REACT_APP_AGOR_API_URL;
...
The matching check is a **strict subset**: if the artifact later adds a new required env var or grant, the existing grant doesn't cover it and the consent modal re-prompts.
...
You can also one-click eject to CodeSandbox (`agor_artifacts_export_codesandbox`) if you want a quick share link. Note that daemon-supplied capabilities won't work there.
...
- **Text and size validation**: Binary files are rejected before transport (instead of being lossy-decoded as UTF-8), and the executor enforces bounded file-count, per-file, total-source, and serialized-request limits below the shared Socket.IO ceiling
...
- [Teammates](/guide/teammates): Persistent AI teammates that can create artifacts as part of their work
