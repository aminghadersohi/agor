...
- `agor_runtime.interactions`: Server-validated action and chat bindings. Action bindings reference existing schedules, which remain the source of truth for prompts, models, tools, and execution policy. Chat bindings reference a canonical session ID rather than a mutable title.
...
### Schedule-backed actions and chat artifacts

An artifact can expose buttons without receiving an API token or choosing an arbitrary prompt at runtime. Declare each action with an artifact-local ID and an existing schedule ID:
...
Artifact code calls `await window.agor.runAction('review')` or `await window.agor.openChat()`. The parent accepts only declared bindings and uses the normal authenticated schedule/session paths, so existing authorization and audit behavior still applies. Schedule and session bindings must belong to the artifact's source branch.
