...
- List an accessible user or teammate's profile gallery with `agor_profile_images_list`, then load a permission-checked small or large image with `agor_profile_images_get` for artifact and visual identity work. Original uploads and storage paths are not exposed.
