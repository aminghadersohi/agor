...
| **Users & Teams**     | Create users, update profiles, inspect permitted user/teammate galleries, and load processed images     |
...
- List an accessible user, teammate, or board image gallery with `agor_profile_images_list`, then load a permission-checked small or large image with `agor_profile_images_get` for artifacts and visual identity work. Original uploads and storage paths are not exposed.
...
`agor_session_relationships_relay(destination, message)` is the safe manual
