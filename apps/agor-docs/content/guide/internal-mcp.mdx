...
## Session self-memory and reminders

The fork POC adds `agor_session_memory_create/list/search/update/archive` and
`agor_session_reminders_create/list/update/cancel`. These tools are deliberately
self-scoped: their schemas contain no target Session ID, and the authenticated
MCP invocation supplies Session, user, and tenant identity. See
[Session memory and reminders](/guide/session-memory-reminders) for limits and
delivery semantics.
...
Every prompt an MCP caller delivers — `agor_sessions_prompt` in any mode,
`agor_session_relationships_report`, the initial prompt of
`agor_sessions_create` / `agor_sessions_spawn`, and an agent-fired zone
trigger — arrives at the recipient with an `<agor_prompt_provenance>` block
in front of it (behind the body, when the body is a slash command so provider
dispatch still works).

The block is stamped by the daemon at admission, from the authenticated
request. Nothing on the wire contributes to it: a caller-supplied
`prompt_provenance` metadata field is discarded unconditionally, and the
`<agor_prompt_provenance>` tag is escaped out of every admitted prompt body, so
the tag can only appear where Agor put it. The durable record is
`Task.metadata.prompt_provenance`; the rendered text is a projection of it.

It names the origin Agor session, its branch and teammate, the Agor user
account, the delivering tool, and how the origin identity was established — a
signed session MCP token, or a personal API key that named a session it can
read through `X-Agor-Session-Id`. Those two are not the same strength and are
not rendered as though they were. A caller that authenticates with a personal
API key and names no session is still admitted, and renders as "origin not
established".

Two things the block deliberately does not claim, because no code path carries
them:

- **It is not human approval.** The Agor user is the account the *calling
  agent* ran under. A relay saying a person authorized something is that
  agent's assertion, not Agor's.
- **It authenticates the hop, not the content.** A genuine session that
  ingested a poisoned page, issue body, or tool result emits a genuinely
  stamped malicious relay.

Prompts typed in the Agor UI and prompts from daemon-internal producers
(callbacks, schedules, session reminders, widget resumes) carry no block. That
absence is meaningful in one direction only: an MCP caller cannot produce it,
because the stamp comes from the request context rather than from any tool
argument.

Inbound gateway messages are a separate mechanism with its own context block,
and Slack follow-ups still carry no provenance block at all. Retrofitting the
gateway header onto this reservation is not part of this contract yet.
...
### Child-session archival policy

`agor_sessions_spawn` and the `subsession`/`btw` modes of `agor_sessions_prompt`
accept `autoArchive` (`never` or `after_completion`) plus a positive
`autoArchiveAfterSeconds`. `agor_sessions_create` accepts the same pair when it
creates a branch-local genealogy child, and `agor_sessions_update` changes the
policy on an existing child. Responses include the persisted snake-case
`auto_archive`, `auto_archive_after_seconds`, and `auto_archive_at` fields.

The defaults are five minutes for BTW forks and one hour for spawned children.
Root Sessions are never automatically archived. Archival is reversible and
does not delete Session data or branch/worktree state.

## Session transfer operations

Two intentionally separate tools move existing Session relationships:

- `agor_sessions_retarget_callback(sessionId, callbackSessionId)` changes a
  standing/direct completion destination. Matching cross-branch
  `remote_create` rows change in the same transaction, and the response returns
  the resolved source, previous destination, new destination, and relationship
  IDs. Callback enablement, mode, delivery policy, template, and include flags are preserved.
- `agor_sessions_reparent(sessionId, parentSessionId)` changes branch-local
  genealogy only. The nullable parent remains in the same branch, both parent
  child lists are updated atomically, and direct or indirect cycles fail.

Both require Manager authority over the source, prompt authority over a new
destination, and a same-tenant active destination. Archived or missing Sessions
and archived or missing destination branches fail closed.

Completion dispatch re-reads the worker Session after a terminal Task commits.
If a direct retarget committed while that Task was running, its standing report
goes only to the new direct destination. Exact-Task callbacks created by
`agor_sessions_prompt(..., callback: true)` are separate immutable Task
subscriptions—including requests propagated from a root orchestrator—and are
not rewritten by direct retargeting.

`agor_sessions_create`, `agor_sessions_spawn`,
`agor_sessions_prompt(mode:"subsession")`, and `agor_sessions_update` expose
`callbackDelivery: "direct" | "btw" | "auto"` for standing callbacks. Omission
means backward-compatible direct delivery. BTW is preflighted against current
destination/branch state, prompt authority, active agent fork capability, and
forkable SDK state. `auto` selects BTW only for a busy destination or a rendered
callback of at least 8 KiB. Unavailable or failed BTW creation falls back to a
deterministic direct Task with an audited reason; it does not drop the callback.
The digest answer is hard-capped at 4 KiB, and deterministic side Session/Task
and final Message IDs fence HA retries. Digest Sessions cannot recursively
deliver callbacks. Exact-Task `callback:true` subscriptions remain direct and
outside this policy, as do root-propagated variants.

`agor_sessions_get_current_context` exposes three intentionally distinct
relationships: current branch-local `parent_session_id`, immutable
`remote_origins`, and the currently enabled
`effective_direct_callback_coordinator_session_id`. A retarget updates the last
field but never rewrites provenance; a reparent updates the first. The context
also returns `effective_callback_delivery` (`direct` when omitted on storage).

`agor_session_relationships_report(destination, message)` is the safe manual
relay path. `destination` must be `parent` or `coordinator`; no arbitrary target
ID is accepted. If both exist, the enum makes the choice explicit. The daemon
resolves current durable state on every call and fails closed when the selected
relationship is missing or disabled, the destination is archived/deleted, the
tenant differs, or the caller cannot prompt the destination.

`agor_sessions_interrupt_with_message(targetSessionId, relationship, message,
idempotencyKey)` is the narrow controlling-direction operation. The calling MCP
Session must be the target's current branch-local parent or enabled direct
callback coordinator; the supplied target ID grants no authority. Agor also
rechecks source visibility, target prompt authority, tenant, active Session and
branch state, and the authenticated human actor at durable admission.

For an active target, Agor atomically requests Stop and inserts one corrective
Task ahead of ordinary queued prompts. That Task cannot start until the normal
termination coordinator verifies executor quiescence/process absence and
settles the stopped Task. Awaiting-permission and legacy awaiting-input work use
the same Stop path. Pending or unverified containment leaves the correction
durably queued; an idle target simply queues it without a fake Stop. Reusing an
`idempotencyKey` for the same caller, target, and exact message returns the same
corrective Task. Stopped turns do not fire natural-completion callbacks; the
correction's eventual completion follows normal callback routing.

`agor_sessions_batch_queue(targetSessionId, relationship, action, ...)` is the
unclaimed-queue counterpart. The calling MCP Session must again be the target's
current parent or enabled direct callback coordinator; a target ID grants no
authority. Call `action: "preview"` first. It returns `queue_revision`, the
exact ordered `expected_task_ids`, compatibility counts, and the deterministic
combined prompt. Then call `action: "combine"` or `action: "replace"` with
those fences and a stable `idempotencyKey`.

Combine preserves every distinct instruction and removes only normalized
duplicates. Replace requires `replacementPrompt` and sends only that canonical
text. Both retain the original Tasks and full request provenance, produce one
execution Task and one aggregate callback, and refuse mixed callback/control or
attachment/widget/gateway/internal semantics. A queue or relationship change
invalidates the preview. This tool never stops running work; use
`agor_sessions_interrupt_with_message` for that case.

The personal fork POC also exposes
`agor_sessions_edit_queued_prompt(targetSessionId, taskId, authority, action,
...)`. Call `preview` first, then pass its exact `queue_revision` and
`prompt_revision` to `update` or `cancel` with a stable `idempotencyKey`.
`author` authority requires the Task's immutable creator. `parent` and
`coordinator` derive authority from the calling MCP Session and recheck that
relationship at commit. Update appends a complete durable revision and changes
only the Task's canonical executor text; cancel records audit and settles it as
`STOPPED`. Once dispatch claims the Task, the operation refuses and callers
must use `interrupt_with_message` for running work. Unsafe delivery classes,
attachments, controls, and over-limit text/history are refused without
truncation or copying prompt text into operational logs.
