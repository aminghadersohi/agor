...
For `agor_boards_auto_arrange_zone`, an explicit `columns` value is exact: Agor never substitutes another column count. If a compact, fully separated grid at that width cannot fit, the tool returns `applied: false`, reports the required dimensions, and leaves every position unchanged. A deliberate `overflowStrategy: "deck"` is available for the rare case where visible-header overlap is preferable to no arrangement.
