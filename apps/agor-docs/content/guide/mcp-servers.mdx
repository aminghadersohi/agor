...
Open **MCP Catalog** from the header’s shop icon or the same icon in the session’s **MCP** popover. Search the session picker to find existing servers; its no-match action also opens the Catalog. Catalog Connect creates a **new session**, not an attachment to the current conversation. To attach a saved server to the current conversation, select it in that picker.
...
- **Open:** Agor creates and attaches a new idle session, stages the starter prompt, and keeps the Catalog drawer open so you can choose **Open session**.
- **OAuth:** Agor creates the server and session and pre-opens the provider sign-in window. The Catalog drawer remains at **Sign-in pending** until the durable per-user grant is confirmed; the session notice and MCP badge remain the recovery path if sign-in is not completed.
