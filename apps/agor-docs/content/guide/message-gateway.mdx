...
> Currently supports **Slack**, **Discord**, **GitHub**, **Shortcut**, and **Microsoft Teams**. WhatsApp and Telegram connectors are planned.
...
> SQLite until its ingress and reply-routing contract is redesigned.
...
delivery path are durable. Responses remain text-only and expose no agent
tools (`agent_tools:[]`). Inbound images are opt-in (`files:true`) and are
