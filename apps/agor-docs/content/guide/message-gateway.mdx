...
> Currently supports **Slack**, **Discord**, **GitHub**, **Shortcut**, and **Microsoft Teams**.
...
Standalone SQLite installations use process-local listeners.

> **Provider limit in PostgreSQL mode:** Slack, GitHub, Shortcut, and Discord listeners
> support distributed ownership. Microsoft Teams currently depends on a local
> Bot Framework port and in-memory conversation references, so Agor fails its
> listener closed on PostgreSQL. Teams remains available only in standalone
> SQLite.

On PostgreSQL, a renewable 30-second lease selects one listener owner per
channel. After an owner dies, failover also takes discovery and provider
connection time. Configuration or credential changes, disabling, and deletion
revoke the old owner. Graceful shutdown allows up to five seconds for admitted
callbacks to drain; if draining is incomplete, the lease expires normally.

Listeners, callbacks, and outbound replies remain bound to the channel’s tenant.
Credentials and routing are revalidated in that tenant; missing or inconsistent
metadata never falls back to a default tenant. A channel change in one tenant
does not stop another tenant’s listeners.
...
Migration `9021_gateway_outbound_thread_seed` moves outbound thread uniqueness
onto `seed_thread_id` and is an **offline cutover**: an older daemon writes no
seed key, so a thread it seeds afterwards stops routing inbound replies. On an
existing database, stop every daemon, run `agor db migrate --offline-cutover`,
then start the upgraded daemons. Rollback is not compatible — a database
migrated here must not be served by an older binary.
...
### Outbound seeds and the audit trail

Every proactive emit writes its own audit row, including a second or third emit
into a thread an earlier emit already started. Exactly one of those rows is the
thread's **seed** — the row a human reply admits to decide which session the
thread belongs to — and it is the one carrying `seed_thread_id`. Later sends
into the same thread are recorded with `seed_thread_id` unset, so they keep
their own provider message id, permalink, text and attribution without
disturbing the thread's existing routing. Which send becomes the seed is
decided by the one-seed-per-thread unique index at insert time, so simultaneous
emits into one thread cannot both claim it.

The emit result reports delivery and persistence separately:

| Field                                       | Meaning                                                                       |
| ------------------------------------------- | ----------------------------------------------------------------------------- |
| `success` / `delivered`                     | The provider accepted this message. A failure to deliver is an error instead. |
| `recorded`                                  | Whether the audit row for this send was persisted.                            |
| `audit_role`                                | `thread_seed`, `thread_followup`, or `unrecorded`.                            |
| `platform_message_id`, `platform_permalink` | Always describe this send, never the thread's existing seed.                  |

A result with `recorded: false` still means the message is in Slack or Discord.
Re-sending it posts a duplicate; reconcile the audit trail instead.
...
Filters select root Sessions. The dry run reports additional active same-branch forked or spawned descendants and how many of them are still executing. If descendants exist, execution requires an explicit choice: `includeChildren: true` archives the complete local trees, while `includeChildren: false` intentionally archives only the matching roots. Archiving is a visibility change, not a stop: a tree that still owns unfinished Tasks is skipped and reported rather than hidden, so stop it with `agor_sessions_stop` first.

Remote-created Sessions live on another branch and retain an independent lifecycle. Bulk archive never follows those cross-branch provenance links.
...
- The channel and author boundaries are separate. Public parent channels,
  ...
  channel, bot/self messages, webhooks, embeds, components, stickers, polls,
  and messages without a structured mention are ignored. Attachment handling
  is controlled separately by the explicit `files` capability below.
- `files:false` is the default text-only behavior. With the explicit
  `files:true` capability, a live mentioned message may include PNG/JPEG
  attachments; each accepted image is downloaded from Discord's signed CDN URL
  into the existing tenant/session/branch upload staging and exposed to the
  remote executor through its normal materialization tool. Unsupported or
  mixed attachment/rich payloads are rejected rather than silently dropping
  content. Per-file and per-message count/size limits still apply; failed or
  expired downloads deliver the text with a bounded degradation note.
...
delivery path are durable. Responses remain text-only. Agent tools are off
unless enabled per row (see Agent channel history below). Inbound images are opt-in (`files:true`) and are
materialized through the normal executor boundary; they are not conversation
history. Text is split into Discord-sized chunks;
...
### Agent channel history

To let session agents read recent Discord messages, enable **Let session agents
read channel history** in Settings → Gateway Channels, or set `channelHistory`
in `agor_gateway_discord_setup`. The capability is off by default
(`agent_tools: { channel_history: true }` enables it).

Agents use `agor_gateway_discord_channel_history_get`:

- Reads are limited to allowlisted public parent channels and public threads
  under them. The bot needs View Channel and Read Message History permissions.
- In a session created by this gateway channel, the default target is the parent
  channel of the session's thread. Other sessions on its target branch pass
  `gatewayChannelId` and `discordChannelId`. Callers without session context
  need admin role or `all` branch permission.
- Results are chronological, with 50 messages by default and at most 200. Page
  with either a `before` or an `after` message-ID cursor. Bot and system messages
  are omitted unless `includeBotMessages` is true. Reads are bounded by the
  gateway channel's catch-up limits; use `next_cursor` when `has_more` is true.

Enabling this gives eligible sessions access through the bot, regardless of the
Agor user's own Discord access. The bot token stays private, and message text
is labeled untrusted. Returned messages become part of the calling session's
transcript; Agor keeps no separate copy of Discord history.
...
3. Configure the explicit channel and user/role allowlists, thread mode,
   ...
   (or opt in to bounded PNG/JPEG inbound images with `files:true`),
   agent tools (off by default), and optional proactive target. Enter the bot token through
...
> **PostgreSQL multi-daemon limitation:** Teams listener startup currently
> fails closed on PostgreSQL. Its webhook server and proactive-reply
> `ConversationReference` are process-local, so the connector cannot safely
> support load-balanced replica ingress yet. Teams remains available in the
> standalone SQLite deployment path. Use Slack, GitHub, or Shortcut for a
> shared PostgreSQL daemon fleet until durable Teams ingress/routing lands.
...
- **Connector Layer.** Platform-agnostic `GatewayConnector` interface with a registry pattern. Currently: `SlackConnector` (Socket Mode), `DiscordConnector` (Discord Gateway), `GitHubConnector` and `ShortcutConnector` (API polling), and `TeamsConnector` (Bot Framework webhook). Adding a new platform means implementing `sendMessage()` and optionally `sendDirectMessage()` and `startListening()`.
...
- **Content is narrow and explicit.** DMs, private threads, embeds, components, stickers, polls, webhooks, and bot/self messages are outside the contract. `files:false` keeps the channel text-only; opt-in `files:true` admits only PNG/JPEG attachments on a mentioned text message, staged through the existing owner-bound upload/materialization path.
...
- [Spawned Subsessions](/guide/sessions#spawned-subsessions-with-callbacks). Gateway-created sessions can themselves spawn child sessions for parallel work.
