...
In daemon HA mode, lifecycle actions remain webhook-only; Agor does not assign
distributed ownership for Start, Stop, Restart, or Nuke. Health observation is
separate: every replica scans active environments, while PostgreSQL admits only
one live observation claim per branch and fences the result against lease expiry
and lifecycle changes. A remote/webhook environment without `health` remains
...
- [Multiplayer and Unix isolation](/guide/multiplayer-unix-isolation) covers how RBAC and Unix user modes gate who can Start / Render / edit env config.
