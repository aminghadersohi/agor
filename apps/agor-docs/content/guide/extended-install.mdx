...
| Platform                | Running Agor locally                                                          |
| ----------------------- | ----------------------------------------------------------------------------- |
| Linux                   | Supported; sandbox mode requires bubblewrap and user namespaces.              |
| macOS                   | Supported in trusted `simple` mode; no local sandbox mode.                    |
| Native Windows          | Unsupported. The `agor-live` npm package blocks installation.                 |
| WSL2 / Linux containers | Not independently validated. Use Linux Node and meet the Linux prerequisites. |

**Windows users can access an existing Agor server in a browser** without installing
anything locally. The separate `@agor-live/client` package provides remote API access,
but native Windows use has not been validated. Other operating systems are untested.
See [execution modes](/guide/multiplayer-unix-isolation) for isolation requirements.

### Diagnosing repository clone failures

Remote repository creation is asynchronous. Poll `agor_repos_get` with the returned
`repo_id` until `clone_status` is `ready` or `failed`. On failure, `clone_error.message`
includes credential-redacted multiline Git diagnostics (up to 4,000 characters;
oversized output retains the tail and marks the truncation), not just the
`Cloning into ...` progress line. Existing failed rows keep their recorded diagnostics;
retry to capture a new error after upgrading.

`not_found` is Git's reported result, not proof that a repository is absent or
public. Check the URL and the calling user's Git credentials and repository access.
Agor does not automatically retry anonymously after an authenticated failure.
After correcting access or Git/CA prerequisites, repeat `agor_repos_create_remote`
with the same slug: the failed database row is replaced and a new `repo_id` is
returned. A ready or in-progress repository is not replaced. Retrying does not
delete repository files; if Git reports a nonempty invalid destination, have an
operator inspect it before removing anything.
...
If the PTY runtime is unavailable, the web terminal links here instead of failing Agor startup.
Retry the install without omitting optional dependencies:
...
- `9004_board_branch_capability_policies` (both dialects) replaces board and branch RBAC with normalized capability policies. Its best-effort conversion is intentionally equal-or-less privilege: legacy prompt sharing is reduced to independent collaboration, and rules that cannot be represented safely may lose access rather than gain it.
- `9012_claude_oauth_attempts` (both dialects) moves Claude paste-back attempts and credential-file mutations into generation-fenced authority. Old daemons do not participate in that protocol and must not overlap the new cohort.
- `9015_mcp_oauth_client_registrations` (PostgreSQL only) moves Dynamic Client Registration ownership into encrypted UUID/CAS authority. SQLite deliberately keeps process-local registration and has no matching migration.
- `9017_fork_migration_collision_repair` (PostgreSQL only) repairs fork auto-archive and zone-workflow schema that a historical OAuth timestamp watermark could otherwise cause Drizzle to skip. It preserves complete existing relations and fails closed on partial shapes.
- `0105_mcp_oauth_grant_attribution` (both databases) retires historical shared MCP OAuth grants while preserving per-user grants. Follow the [shared-grant upgrade checklist](/guide/mcp-servers#upgrade-shared-grant-attribution-0105-both-databases) for inventory, quiescing exchanges/refreshes, reauthorization ownership, post-cutover verification, and backup restoration limits.
...
The in-app **Sign in with Claude** flow is enabled by default only when a supported
storage mode is operational. Operators may opt out with
`agentic_tools.claude_subscription_oauth: false`; provider approval remains a
separate release prerequisite. See [Operator configuration](./config-yaml).

Contained local sandbox deployments retain daemon-owned credential files and
unchanged isolation gates. Supported delegated PostgreSQL deployments instead
keep encrypted personal grants in the backend. Refresh credentials never reach
agent/helper runtimes: only a task actor's access token is delivered through the
existing authenticated task RPC. API keys and pasted tokens remain available.

The additive `0110_user_provider_oauth_grants` schema introduces an incompatible
source/attempt protocol and requires the offline stop → migrate → start procedure
above. No mixed old/new writer cohort or automatic filesystem import is supported.
Tenant re-home and restored historical rotating grants require reconnect.

Refresh happens before a new task starts. A long-running task can outlive its
access token; send a follow-up prompt in the same session after an expiry failure
to start a new task while preserving SDK resume state. No automatic prompt replay or billing fallback occurs.
Disconnect prevents new token delivery but cannot revoke already-delivered bearers.
...
token against the catalog URL and writes the server only after the probe accepts it. A stale
...
GitHub is a narrow reviewed exception to that OAuth rule. Its remote endpoint challenges for OAuth but
does not publish Dynamic Client Registration, so Marketplace requests a fine-grained personal access
token through GitHub's documented bearer route and verifies it against the pinned catalog endpoint
before storage. Scope the token to only the repositories and operations needed; organisation policy can
restrict PATs, Enterprise Managed User PATs are disabled by default, and individual tools can still
require paid GitHub or Copilot features.

Prisma, MongoDB, Box, HubSpot, Slack, PagerDuty, and Kagi remain absent because review could not
establish a safely bound client-registration or issuer path. Their absence is not the earlier state where
OAuth cards were offered but failed after Connect; unsupported providers are removed from the shelf
instead.
...
entry reuses only that user's catalog connection and rotates the token after the authenticated probe
has succeeded.

Every successful Connect adds the server without creating a session. The drawer shows a compact next
step: keep browsing, or choose **Start new session** and select an eligible teammate and agent tool.
That explicit action creates a caller-owned idle session through the normal session service, attaches
the added server, and seeds the starter prompt into the composer as editable, unsent text. A new OAuth
connection still pre-opens the provider window while browser user activation is available. The drawer
stays at **Sign-in pending** until Agor confirms the durable grant; navigating the popup is not treated
as success. If sign-in does not finish, **My Servers** remains the recovery path.
