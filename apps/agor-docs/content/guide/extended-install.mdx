...
- `9018_attention_watermark_collision_repair` (PostgreSQL only) repairs private per-user attention state if the archived DCR watermark caused its private-band migration to be skipped. It also preserves complete forced-RLS state and fails closed on partial shapes.
