...
export function useConversationHistory(
  ...
  const [boundary, setBoundary] = useState<Boundary | null>(() => {
    return tasks.length ? boundaryFor(sessionId, tasks.slice(-pageSize)) : null;
  });
  ...
  const startIndex = useMemo(() => {
    ...
    return index === -1 ? Math.max(0, tasks.length - pageSize) : index;
  ...
  const revealOlder = useCallback(() => {
    const next = tasks.slice(Math.max(0, startIndex - pageSize));
