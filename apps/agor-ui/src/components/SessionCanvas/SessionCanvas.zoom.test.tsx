...
// board presence socket, so a client stub that answers only `patch` leaves
// `find`/`on` undefined.
...
describe('SessionCanvas zoom shortcuts', () => {
  ...
  describe('onNodesChange zone resize via O(1) getNode lookup', () => {
    ...
    function makeClient() {
      // PR #2540's layout-aware patch, behind the fork's fuller client shape:
      // SessionCanvas subscribes to zone-workflow-transitions and to io, so the
      // one-service stub would leave those undefined.
      const patch = createLayoutPatch();
