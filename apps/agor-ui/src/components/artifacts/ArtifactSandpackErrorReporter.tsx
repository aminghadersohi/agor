...
export function ArtifactSandpackErrorReporter({
  ...
  artifactId: string;
  contentHash?: string;
  /** Effective lifecycle status for static previews without a Sandpack client. */
  ...
  const compilationStatus: ArtifactCompilationStatus = sandpack.error
    ...
    : statusOverride
      ...
      : sandpack.status !== 'running' || compilation.contentHash !== contentHash
        ? 'pending'
        : compilation.status;
  const runtimeStatus = statusOverride ?? sandpack.status;
  ...
  }, [sandpack.error, runtimeStatus, compilationStatus, artifactId, contentHash]);
