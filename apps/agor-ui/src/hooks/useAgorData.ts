...
export function useAgorData(
  ...
  useEffect(() => {
    ...
    const sessionPatchedBatched = (session: Session) => {
      if (!subscriptionIsCurrent()) return;
      // Board counts track every event as it arrives. Lifecycle keeps its own
      // `previous`: a held terminal downgrade must stay "previously executing"
      // across duplicate events, so count bookkeeping must not overwrite it.
      const countPrevious =
